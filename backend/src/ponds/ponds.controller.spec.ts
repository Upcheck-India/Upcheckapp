...
describe('PondsController', () => {
  ...
  beforeEach(async () => {
    service = {
      ...
      archive: jest
        .fn()
        .mockResolvedValue({ message: 'Pond archived successfully' }),
      remove: jest
        .fn()
        .mockResolvedValue({ message: 'Pond deleted successfully' }),
  ...
  describe('PATCH /ponds/:id/archive', () => {
    ...
    it('should propagate ConflictException for active cycle', async () => {
      ...
      await expect(controller.archive('pond-1', mockUser)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe('GET /ponds/:id/dimension-history', () => {
    it('should return dimension history', async () => {
      ...
      const result = await controller.getDimensionHistory(
        'pond-1',
        mockUser,
        pageOptions,
      );
      expect(service.getDimensionHistory).toHaveBeenCalledWith(
        'pond-1',
        'user-1',
        pageOptions,
      );
