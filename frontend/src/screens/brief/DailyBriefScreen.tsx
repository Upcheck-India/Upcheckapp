...
export const DailyBriefScreen = ({ navigation, route }: any) => {
    ...
    const blocks = (b: DailyBrief) => {
        ...
        switch (mode) {
            case 'morning':
                return [carried, todo, happening, ponds, numbers];
            case 'soFar':
                return [todo, carried, ponds, happening, numbers];
            default:
                return [numbers, ponds, todo, carried, happening];
        }
    };
            ...
            <>
                ...
                {brief.hasAnyData && (
                    <View style={styles.actions}>
                        <TouchableOpacity style={styles.button} onPress={() => run('pdf')} disabled={!!busy} accessibilityRole="button" accessibilityState={{ busy: busy === 'pdf' }}>
                            <Icon name="receipt_long" size={18} color={c.textPrimary} />
                            <Text style={styles.buttonText}>{t('dailyBrief.actions.exportPdf')}</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.button} onPress={() => run('image')} disabled={!!busy} accessibilityRole="button" accessibilityState={{ busy: busy === 'image' }}>
                            <Icon name="share" size={18} color={c.textPrimary} />
                            <Text style={styles.buttonText}>{t('dailyBrief.actions.shareImage')}</Text>
                        </TouchableOpacity>
                    </View>
                )}
