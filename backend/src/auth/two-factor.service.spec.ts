...
describe('TwoFactorService — backup codes (AUTH-4)', () => {
    ...
    const repo = {
        findOneBy: jest.fn(async ({ id }: { id: string }) => (user && user.id === id ? user : null)),
    ...
    it('enable() returns 10 backup codes and stores only hashes', async () => {
        ...
        expect(user.backupCodes).not.toEqual(expect.arrayContaining(res.backupCodes));
    ...
    it('regenerate replaces the codes and requires a valid TOTP', async () => {
        ...
        await expect(service.regenerateBackupCodes(user.id, '000000')).rejects.toBeInstanceOf(
        ...
        // A new one does.
        expect(await service.verifyCodeOrBackup(user.id, backupCodes[0])).toBe(true);
    });
