...
| Harvest plans | `harvest-plans` → `POST`, `GET`, `GET :id`, `PATCH :id`, `PATCH :id/complete`, `DELETE` | `harvest/HarvestPlansScreen` | `harvest_plans`, `harvests.plan_id` | One revenue path (H4): "Mark complete" opens HarvestLog prefilled from the plan; saving the harvest (`planId` on `POST /harvests`) completes the plan in the same transaction, only if the plan is on the crop's pond (else 400 `PLAN_WRONG_POND`) and still planned (else 409 `PLAN_ALREADY_COMPLETED`). A plan's `cropId` must be its pond's active cycle; PATCH cannot change pond or crop. Complete and delete are RECORD_HARVEST. Prices/revenue masked without VIEW_FINANCIALS. **`PATCH :id/complete` is a compatibility shim for pre-H4 app builds**: it logs a single-grade full harvest through `HarvestsService.create` and writes no transaction — **make it 410 Gone `USE_HARVEST_LOG` in the next release.** Old plan-completion `transactions` rows (`harvest_sale`) are left as they are; the Money overview flags a cycle that also has a harvest (`possibleDuplicateHarvestIncome`) instead of rewriting money. Needs migration `1780701000000`. |
