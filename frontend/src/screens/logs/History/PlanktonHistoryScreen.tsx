...
export const PlanktonHistoryScreen = ({ route, navigation }: any) => {
    ...
    const { data, isInitialLoading: isLoading, isRefreshing, error, refresh: handleRefresh, refetch: fetchRecords, setData } = useCachedFetch(
        ['planktonHistory', cropId ?? null],
        async (): Promise<PlanktonRecord[]> => {
            if (!cropId) return [];
    ...
    const setRecords = (fn: (prev: PlanktonRecord[]) => PlanktonRecord[]) => setData((prev) => fn(prev ?? []));

    const handleDelete = useCallback((item: PlanktonRecord) => {
        Alert.alert(
            t('common.delete') + ' ' + t('common.date'),
            t('history.planktonDeleteMsg', { date: formatDate(item.measurementDate, { day: 'numeric', month: 'short', year: 'numeric' }) }),
            [
