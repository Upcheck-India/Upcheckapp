...
export class ReferenceService {
  ...
  async findOneHatchery(id: string): Promise<Hatchery> {
    ...
    if (!hatchery)
      throw new NotFoundException(`Hatchery with ID ${id} not found`);
    return hatchery;
  }
  ...
  async findOneBroodstock(id: string): Promise<Broodstock> {
    ...
    if (!broodstock)
      throw new NotFoundException(`Broodstock with ID ${id} not found`);
    return broodstock;
  }

  async updateBroodstock(
    id: string,
    dto: UpdateBroodstockDto,
  ): Promise<Broodstock> {
