...
export class PondDimensionService {
  ...
  calculateArea(geometryType: string, dimensions: PondDimensions): number {
    ...
    switch (geometryType) {
      case 'rectangular':
        if (!dimensions.lengthM || !dimensions.widthM) {
          throw new BadRequestException(
            'Rectangular ponds require length and width',
          );
        }
      ...
      case 'raceway':
        if (!dimensions.lengthM || !dimensions.widthM) {
          throw new BadRequestException(
            'Raceway ponds require length and width',
          );
        }
  ...
  validateDimensions(geometryType: string, dimensions: PondDimensions): void {
    if (dimensions.depthM < PondDimensionService.DEPTH_MIN) {
      throw new BadRequestException(
        `Depth must be at least ${PondDimensionService.DEPTH_MIN}m`,
      );
    }
    if (dimensions.depthM > PondDimensionService.DEPTH_MAX) {
      throw new BadRequestException(
        `Depth must not exceed ${PondDimensionService.DEPTH_MAX}m`,
      );
    }
    ...
    if (area > 0 && area < PondDimensionService.AREA_MIN) {
      throw new BadRequestException(
        `Calculated area ${area}m² is below minimum of ${PondDimensionService.AREA_MIN}m²`,
      );
    }
    if (area > PondDimensionService.AREA_MAX) {
      throw new BadRequestException(
        `Calculated area ${area}m² exceeds maximum of ${PondDimensionService.AREA_MAX}m²`,
      );
    }
  }
  ...
  hasDimensionsChanged(
    ...
    return (
      ...
      (update.diameterM !== undefined &&
        update.diameterM !== oldPond.diameterM) ||
      (update.depthM !== undefined && update.depthM !== oldPond.depthM) ||
      (update.overrideAreaM2 !== undefined &&
        update.overrideAreaM2 !== oldPond.overrideAreaM2)
    );
  }
}
