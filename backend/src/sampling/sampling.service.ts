...
export class SamplingService {
  ...
  async create(createDto: CreateSamplingDto, userId: string) {
    ...
    if (createDto.id) {
      const existing = await this.samplingRepository.findOne({
  ...
  async findOne(id: string): Promise<SamplingData> {
    ...
    if (!record)
      throw new NotFoundException(`Sampling data with ID ${id} not found`);
    return record;
  }

  async update(
    id: string,
    updateDto: UpdateSamplingDto,
    userId?: string,
  ): Promise<SamplingData> {
    await this.findOne(id);
    await this.samplingRepository.update(id, {
      ...updateDto,
