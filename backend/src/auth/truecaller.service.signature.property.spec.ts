...
describe('property: signature verification', () => {
  ...
  it('accepts well-formed payloads and rejects every adversarial mutation', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
        ...
        async (params) => {
          ...
          const buildPayload = (nonce: string, requestTime: number): string => {
            const profile = {
              requestNonce: nonce,
              requestTime,
            ...
            return Buffer.from(JSON.stringify(profile), 'utf-8').toString(
              'base64',
            );
          };
